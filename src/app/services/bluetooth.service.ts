...
import {filter, from, fromEvent, map, mergeMap, Observable, of, takeUntil} from "rxjs";
...
export class OLDBluetoothService {
  ...
  async value(options: ReadValueOptions) {
    console.log('[BLE::Info] Reading value with options %o', options);
    ...
    try {
      ...
      console.log('[BLE::Info] Value info %o', value);
   ...
   */
  discover$(options?: RequestDeviceOptions): Observable<BluetoothRemoteGATTServer | undefined | null> {
  ...
  disconnectDevice() {
    ...
    console.log('[BLE::Info] Disconnecting from Bluetooth Device %o', this.gattServer);

    if (this.gattServer.connected) {
      this.gattServer.disconnect();
    } else {
      console.log('[BLE::Info] Bluetooth device is already disconnected');
    }
  }
  ...
  getPrimaryService$(gatt: BluetoothRemoteGATTServer, service: BluetoothServiceUUID): Observable<BluetoothRemoteGATTService> {
    console.log('[BLE::Info] Getting primary service "%s" (if available) of %o', service, gatt);
    ...
    if (gatt) {
      ...
      console.error('[BLE::Error] Was not able to connect to the Bluetooth Remote GATT Server');
    }
    return of()
  }
